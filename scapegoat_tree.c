...
node* BST_build(int k,node * groot){
     node * u;
     if(groot==NULL){
        groot=allocate(k);
     }
     else if(k<groot->value){
        groot->left=BST_build(k,groot->left);
        groot->left->parent=groot;
     }
     else{
        groot->right=BST_build(k,groot->right);
...
void add(int k, node * root){printf("yes\n");
    n++;q++;                                // increase number of nodes n and counter q
    int depth=0;
    node * temp=root,prev=temp, x;
    x=allocate(k);
    while(temp!=NULL){
    if(temp->value>k){
        prev=temp;
...
int main(){
    node* root=NULL;
    int c,k;
    do{printf("Enter choice (1=insert,2=display,0=END):");
    scanf("%d",&c);
    switch(c){
